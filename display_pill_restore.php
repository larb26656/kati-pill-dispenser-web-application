...
function(){
        var html = $.ajax({
        type: "GET",
        url: "include/pill/restore_pill.php",
        data: info,
        async: false ,
         success: function() {
...
<?php 
include("connect.php");
$sql = "SELECT * FROM `pill` WHERE Pill_visiblestatus = '0' ORDER BY Pill_id";
...
<td>
<button type="button" class="btn btn-success btn-xs" onClick="restore_pill(<?= $row['Pill_id'] ?>)"><?php echo $strTablerecoverybutton;?></button>
</td>
      </tr>
     <?php } ?>
    </tbody>
