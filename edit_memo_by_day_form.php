...
<script type="text/javascript">
   $(document).ready(function () {
        ...
        $('#Memo_notification_date').change(function() {
          select_hours_edit('<?php echo $Memo_id;?>',$("#Memo_notification_date" ).val());
        });
        $( "#hours" ).change(function() {
          select_minutes_edit('<?php echo $Memo_id;?>',$('#hours').val(),$("#Memo_notification_date" ).val());
        });
    $("#update_memo").validate({
       ...
       submitHandler: function(form) {
        var fData = new FormData(document.getElementById("update_memo"));
  $.ajax({
    'type':"POST",
    'url':"include/schedule/update_memo_by_day.php",
    'data': fData,
    'contentType':false,
    'processData':false,
    'cache':false,
  ...
  return false;
    ...
    function select_hours_edit(memo_id,dayofweek){
    var info = 'memo_id=' + memo_id +'&dayofweek=' + dayofweek;
    $.ajax({
      type:"GET",
      data:info,
      url:"include/schedule/select_time_picker_hours_memo_by_day_edit.php",
      success:function(data){
        $("#hours").html(data);
        select_minutes_edit('<?php echo $Memo_id;?>',$("#hours option:selected").val(),$("#Memo_notification_date" ).val());
      }
    });
  }
select_hours_edit('<?php echo $Memo_id;?>',$("#Memo_notification_date" ).val());
 function select_minutes_edit(memo_id,hours,dayofweek){
    var info = 'memo_id=' + memo_id +'&hours=' + hours +'&dayofweek=' + dayofweek;
    $.ajax({
      type:"GET",
      data:info,
      url:"include/schedule/select_time_picker_minutes_memo_by_day_edit.php",
      success:function(data){
        $("#minutes").html(data);
      }
    });
...
<form class="form-horizontal" id="update_memo" name="update_memo">
  <input type="hidden" id="Memo_id" name="Memo_id" value="<?php echo $row['Memo_id'];?>">
   <div class="form-group">
    ...
    <div class="col-sm-4 input-container">
      <input type="date" class="form-control" id="Memo_notification_date" name="Memo_notification_date" value="<?php echo $row['Memo_notification_date'];?>">
    </div>
  </div>
  <div class="form-group">
