...
<script type="text/javascript">
   $(document).ready(function () {
        ...
        $('.display_restore').addClass('active');
...
function(){
        var html = $.ajax({
        type: "GET",
        url: "include/outsider/restore_outsider.php",
        data: info,
        async: false ,
         success: function() {
    window.location.reload(true);}
  ...
  function edit_outsider(id)
{
 var info = 'outsider_id=' + id;
  ...
  confirmButtonColor: "#DD6B55",
  confirmButtonText: "<?php echo $strDialogboxconfirmbutton;?>",
  cancelButtonText: "<?php echo $strDialogboxconfirmbutton;?>",
  closeOnConfirm: false
},
function(){
...
<?php 
include("connect.php");
$sql = "SELECT * FROM `member` WHERE Member_visiblestatus = '0'";
$result=$conn->query($sql); 
$sql2 = "SELECT * FROM `outsider` WHERE Outsider_visiblestatus = '0'";
...
<div id="wrapper">  
  ...
  <li><a href="#" class="display_restore"><i class="fa fa-eye" aria-hidden="true"></i> <?php echo $strSubmenudisplaydeleteinformation;?></a></li>
...
<td>
<button type="button" class="btn btn-success btn-xs" onClick="restore_member(<?= $row['Member_id'] ?>)"><?php echo $strTablerecoverybutton;?></button>
</td>
      </tr>
     <?php } ?>
    </tbody>
...
<td>
<button type="button" class="btn btn-success btn-xs" onClick="restore_outsider(<?= $row2['Outsider_id'] ?>)"><?php echo $strTablerecoverybutton;?></button>
</td>
      </tr>
     <?php } ?>
    </tbody>
