...
function get_hours_list($day_pattern){
	$hours = "0";
	for ($x = 0; $x <= 23; $x++) {
		if(get_all_minute_list($x,$day_pattern)==[]){
		}
		else{
...
function get_time_list_from_memo_database($hours,$day_pattern){
	global $conn;
	$x = 0;
	$sql = "SELECT `Memo_notification_time` AS range_1,ADDTIME(`Memo_notification_time`, SEC_TO_TIME(4*60)) AS range_2 FROM `memo` WHERE `Memo_visiblestatus` = 1 AND SUBSTR(ADDTIME(`Memo_notification_time`, SEC_TO_TIME(4*60)) ,1,2) = '$hours' ".get_memo_sql_condition($day_pattern)." ORDER BY range_1";
	$result=$conn->query($sql); 
	$num_row = mysqli_num_rows($result);
	if($num_row >= 1){
		 while($row=$result->fetch_assoc()){
...
foreach (get_hours_list($day_pattern) as $key => $value) {
	echo "<option value='$value'>".$value."</option>";
	}
	//echo(get_memo_sql_condition("1000001"));
